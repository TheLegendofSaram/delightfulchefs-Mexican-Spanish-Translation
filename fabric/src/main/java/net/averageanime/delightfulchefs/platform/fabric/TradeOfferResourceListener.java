...
public class TradeOfferResourceListener extends JsonDataLoader implements IdentifiableResourceReloadListener {
    ...
    public Identifier getFabricId() {
        return new Identifier(DelightfulChefs.MOD_ID,"villager_data_loader");
    }
    ...
    private void loadAndMergeTradeOffers(ResourcePack pack, VillagerProfession villager) throws IOException {
        ...
        InputSupplier<InputStream> tradeInputStreamSupplier = pack.open(ResourceType.SERVER_DATA, new Identifier(DelightfulChefs.MOD_ID, "villager_trades/" + id + ".json"));

        if (tradeInputStreamSupplier == null || tradeInputStreamSupplier.get() == null) {
            return;
        }

        DelightfulChefs.LOGGER.info("Deserializing datapack added trades of profession: " + DelightfulChefs.MOD_ID + ":" + villager + " from " + pack.getName());
