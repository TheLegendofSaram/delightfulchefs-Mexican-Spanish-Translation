...
import net.averageanime.delightfulchefs.platform.ConfigDirectory;
...
public class OmegaConfig {
        ...
        try {
            ...
            if (!configExists(config)) {
                ...
                try {
                    ...
                    DelightfulChefs.LOGGER.error(e.toString());
                    DelightfulChefs.LOGGER.info(String.format("Encountered an error while reading %s config, falling back to default values.", config.getName()));
                    DelightfulChefs.LOGGER.info(String.format("If this problem persists, delete the config file %s and try again.", config.getName() + "." + config.getExtension()));
                }
            }
    ...
    public static <T extends Config> void writeConfig(Class<T> configClass, T instance) {
        ...
        try {
            ...
            DelightfulChefs.LOGGER.error(ioException.toString());
            DelightfulChefs.LOGGER.info(String.format("Write error, using default values for config %s.", configClass));
        }
    }
