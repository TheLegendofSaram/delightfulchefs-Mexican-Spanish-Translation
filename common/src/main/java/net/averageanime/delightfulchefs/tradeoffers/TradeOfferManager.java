...
import net.averageanime.delightfulchefs.tradeoffers.trades.*;
...
public class TradeOfferManager {
    ...
    public static void registerTradeOffers() {
        DelightfulChefs.LOGGER.info("Registered JSON trade offer adapter.");
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"sell_item"), new JsonSellItemTradeOffer());
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"buy_item"), new JsonBuyItemTradeOffer());
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"process_item"), new JsonProcessItemTradeOffer());
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"sell_potion"), new JsonSellPotionTradeOffer());
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"sell_enchanted_tool"), new JsonSellEnchantedToolTradeOffer());
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"sell_enchanted_book"), new JsonSellEnchantedBookTradeOffer());
        tradeOfferRegistry.put(new Identifier(DelightfulChefs.MOD_ID,"sell_map"), new JsonSellStructureMapTradeOffer());
    }
    ...
    private static void deserializeTrades(@NotNull JsonObject jsonRoot, BiConsumer<Integer, TradeOffers.Factory> tradeConsumer) {
        for (Map.Entry<String, JsonElement> entry : jsonRoot.get("trades").getAsJsonObject().entrySet()) {
            ...
            for (JsonElement tradeElement : tradesArray) {
                ...
                if (adapter == null) {
                    DelightfulChefs.LOGGER.error("Trade type: " + trade.get("type").getAsString() + " is broken.");
                    DelightfulChefs.LOGGER.error("Error in deserializing trades." +
